...
export function useSseStatus(): "live" | "down" {
  ...
  useEffect(() => {
    ...
    // The stream is module-level, so it can reach OPEN in the window between the
    // useState initializer and this effect. Nothing else fires until a
    // disconnect, so without this readyState re-check the LED latches "down" on
    // a healthy connection.
    if (es.readyState === EventSource.OPEN) up();
