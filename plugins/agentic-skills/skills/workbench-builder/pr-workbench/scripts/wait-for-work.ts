/**
 * Wake-on-work watcher — the agent's ear on the workbench.
 *
 * An LLM agent cannot be pushed to; it can only block on a wait. This script
 * IS that wait: the agent launches it as a background command
 * (`run_in_background: true`), and the exit re-invokes the agent with the
 * printed digest as its work order. Relaunch after processing each batch —
 * the relaunch doubles as a health check on the loop.
 *
 * Listens to the server's own SSE invalidation stream and exits (printing a
 * JSON digest to stdout) when EITHER:
 *   - a request is queued (the "Hand batch to agent" button or a typed
 *     question) → reason "request", immediate — human intent skips the wait;
 *   - the board goes quiet for DEBOUNCE_MS with human events sitting past the
 *     watermark → reason "debounce" — the edit-free buffer.
 *
 * Every named SSE event except `queue`/`hello` arms the debounce, so adding a
 * region to server.ts needs no change here. The script only listens and
 * reports — judgment about what the batch means (which PR to re-analyze, which
 * concern to answer) belongs to the agent it wakes.
 *
 * After processing a batch the agent must advance the cursor:
 *   POST /claude/watermark {"last_event_id": <latest_event_id from the digest>}
 *
 * Run: bun run scripts/wait-for-work.ts
 * Env: WORKBENCH_URL (default http://127.0.0.1:5051), WATCH_DEBOUNCE_MS (default 20000)
 */
const BASE = process.env.WORKBENCH_URL ?? "http://127.0.0.1:5051";
const DEBOUNCE_MS = Number(process.env.WATCH_DEBOUNCE_MS ?? 20_000);

type Digest = {
  watermark: number;
  latest_event_id: number;
  events: unknown[];
  open_concerns: unknown[];
  queued_requests: unknown[];
};

async function digest(): Promise<Digest> {
  const r = await fetch(`${BASE}/claude/digest`);
  if (!r.ok) throw new Error(`/claude/digest → ${r.status}`);
  return (await r.json()) as Digest;
}

function finish(reason: "request" | "debounce", d: Digest): never {
  console.log(JSON.stringify({ reason, ...d }, null, 2));
  process.exit(0);
}

/** A queued request always wakes the agent; quiet-period events wake it only
 *  when the debounce timer (not a mid-burst queue event) asked the question. */
async function check(trigger: "debounce" | "queue"): Promise<void> {
  const d = await digest();
  if (d.queued_requests.length > 0) finish("request", d);
  if (trigger === "debounce" && d.events.length > 0) finish("debounce", d);
}

let timer: ReturnType<typeof setTimeout> | null = null;
function arm() {
  if (timer !== null) clearTimeout(timer);
  timer = setTimeout(() => void check("debounce"), DEBOUNCE_MS);
}

// Startup sweep: work that landed while nothing was listening. A queued
// request exits now; pending events get one debounce period in case the human
// is mid-burst.
const d0 = await digest();
if (d0.queued_requests.length > 0) finish("request", d0);
if (d0.events.length > 0) arm();
console.error(`listening on ${BASE}/events — debounce ${DEBOUNCE_MS}ms, watermark ${d0.watermark}`);

const res = await fetch(`${BASE}/events`);
if (!res.ok || res.body === null) throw new Error(`/events → ${res.status}`);
const reader = res.body.getReader();
const decoder = new TextDecoder();
let buf = "";
for (;;) {
  const { done, value } = await reader.read();
  if (done) break;
  buf += decoder.decode(value, { stream: true });
  let nl: number;
  while ((nl = buf.indexOf("\n")) >= 0) {
    const line = buf.slice(0, nl).trim();
    buf = buf.slice(nl + 1);
    if (!line.startsWith("event:")) continue;
    const event = line.slice("event:".length).trim();
    if (event === "queue") {
      await check("queue");
    } else if (event !== "hello") {
      arm();
    }
  }
}
console.error("SSE stream closed — is the server down?");
process.exit(1);
