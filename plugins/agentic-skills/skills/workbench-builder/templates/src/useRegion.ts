...
export function useSseStatus(): "live" | "down" {
  ...
  useEffect(() => {
    ...
    // Subscribe, THEN sync. The module-level EventSource starts connecting at
    // import time, so `open` and `hello` both fire before React commits this
    // effect (measured on loopback: the stream opens ~11ms before the effect
    // runs). Reading readyState only in the useState initializer therefore
    // latches "down" forever on a stream that is in fact live — the LED lies
    // while every region updates correctly around it.
