...
/**
 * Status hues from bridge.css's ramp, keyed by the outcome names the eval rows
 * actually carry (`pending` | `pass` | `fail` — see EvalBoard's Eval type) plus
 * the `passed`/`failed` series names the run-history chart stacks. A key that no
 * caller passes silently falls through to SERIES[0], which paints every slice one
 * colour, so these keys and the callers in App.tsx move together.
 *
 * An outcome keeps its hue everywhere it appears: the donut slice, the stacked
 * run-history bar, and EvalBoard's success/error/neutral Badge all resolve to the
 * same three Graphite status hues.
 * Dark: pending #8593a0, pass #2fb6a4, fail #e0635a.
 */
export const STATUS_COLORS: Record<string, string> = {
  pending: "#586471", // --status-closed grey — not yet run, recedes
  pass: "#0a6961", // --status-done teal
  fail: "#9a3b33", // --status-blocked brick
  passed: "#0a6961", // run-history series name for the same outcome
  failed: "#9a3b33",
};
...
export function stackedShareOption(
  ...
  return {
    ...
    series: names.map((name, i) => ({
      ...
      type: "bar",
      stack: "share",
      // An outcome that has a status hue keeps it here too, so the run-history
      // bar and the outcome donut read as one language; anything else walks the
